...
def save_preprocessed_data(df, X_train, X_test, y_train, y_test, sample_size='10k'):
    ...
    df.to_csv(f'data/preprocessed_{sample_size}.csv', index=False)
    ...
    train_data.to_csv(f'data/train_{sample_size}.csv', index=False)
    test_data.to_csv(f'data/test_{sample_size}.csv', index=False)
...
def main():
    ...
    df = pd.read_csv('data/sample_10k.csv')
